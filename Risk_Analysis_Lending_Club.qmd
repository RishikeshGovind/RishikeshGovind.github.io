---
title: '**Risk Analysis of Lending Club Loans**'
jupyter: python3
format:
  html:
    page-layout: custom
...
```{bash}
pip install seaborn scikit-learn imbalanced-learn chart-studio plotly tensorflow
...
# Use an iframe‐based renderer that Quarto can inline
pio.renderers.default = "iframe_connected"

# Prepare your data as before
charged_off      = loan_status_cross['Charged Off'].tolist()
defaults         = loan_status_cross['Default'].tolist()
not_meet_credit  = loan_status_cross['Does not meet the credit policy. Status:Charged Off'].tolist()
grace_period     = loan_status_cross['In Grace Period'].tolist()
short_pay        = loan_status_cross['Late (16-30 days)'].tolist()
long_pay         = loan_status_cross['Late (31-120 days)'].tolist()

# Build the traces
trace_charged = go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=charged_off, name='Charged Off', text='%')
trace_defaults= go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=defaults,    name='Defaults',    text='%')
trace_ncp     = go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=not_meet_credit, name='Does not meet Credit Policy', text='%')
trace_grace   = go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=grace_period, name='Grace Period', text='%')
trace_short   = go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=short_pay,    name='Late (16-30 days)', text='%')
trace_long    = go.Bar(x=['MidWest','NorthEast','SouthEast','SouthWest','West'],
                      y=long_pay,     name='Late (31-120 days)', text='%')

# Compose figure
fig = go.Figure(
    data=[trace_charged, trace_defaults, trace_ncp, trace_grace, trace_short, trace_long],
    layout=go.Layout(
        barmode='stack',
...
# Show it — Quarto will inline the iframe
fig.show()
...
```{python}
#| colab: {base_uri: https://localhost:8080/, height: 745}
